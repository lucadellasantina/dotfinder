...
for i=1:size(AllSegCut,1)
        ...
        for d=1:devs+1
            ...
            DDm(max(1,ceil(sy*Scxy)), max(1,ceil(sx*Scxy)), max(1,ceil(sz*Scz)))=1; %draw Skel %ceil because um scale starts from 0 but voxel goes 1,2,3,... but somehow some pts still get 0, so needed to add max(1,...) HO 10/6/2010
        end
end
clear Dist
...
%%DrawNodes
for i=1:size(AllSegCut,1)
        DDm(max(1,ceil(AllSegCut(i,1,1)*Scxy)), max(1,ceil(AllSegCut(i,2,1)*Scxy)), max(1,ceil(AllSegCut(i,3,1)*Scz)))=2; %draw nodes %ceil because um scale starts from 0 but voxel goes 1,2,3,... but somehow some pts still get 0, so needed to add max(1,...) HO 10/6/2010
        DDm(max(1,ceil(AllSegCut(i,1,2)*Scxy)), max(1,ceil(AllSegCut(i,2,2)*Scxy)), max(1,ceil(AllSegCut(i,3,2)*Scz)))=2; %draw nodes %ceil because um scale starts from 0 but voxel goes 1,2,3,... but somehow some pts still get 0, so needed to add max(1,...) HO 10/6/2010
end
