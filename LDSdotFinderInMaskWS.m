...
%%
function[]=LDSdotFinderInMaskWS(TPN) %#ok<INUSL>
% 
if ~exist('TPN')
    TPN = GetMyDir;
...
imwrite(Post(:,:,1), [TPN 'find' filesep 'PostMask.tif'], 'tif', 'compression', 'none');
for i=2:size(Post,3)
    imwrite(Post(:,:,i), [TPN 'find' filesep 'PostMask.tif'], 'compression', 'none', 'WriteMode', 'append');
end

%% Enter Variables

v = evalin('base', 'Settings');
v = v.dotfinder;
% 
% %%Image Variables
% % xyum=.103;
...
BackgroundLabel = mode(double(wsTMLabels(:)));
