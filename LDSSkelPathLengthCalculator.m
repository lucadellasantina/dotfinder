function Skel = HOSkelPathLengthCalculator(Skel);
...
%check the result.
disp('Farthest skel path distance is');
max(SkelPathLength2Soma)
disp('Soma point ID is (Imaris soma pt ID + 1)');
SomaPtID
disp('Skel IDs of zero path distance are (only soma point should have zero path distance)');
find(SkelPathLength2Soma==0)
...
%put the result under Skel.FilStats
