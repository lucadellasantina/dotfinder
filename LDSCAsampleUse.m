function [CA] = LDSCAsampleUse(TPN)
...
%if exist([TPN 'find\SG.mat']) %SG should exist, so I took it out 10/15/2011 HO
...
%Idir=dir([TPN 'mask']); %HO 7/8/2010
%IMtest=imread([TPN 'mask\' Idir(3).name]); %HO 7/8/2010
%[yM xM]=size(IMtest); %HO 7/8/2010
load([TPN 'Settings.mat']) %HO 7/8/2010
ImageInfo = evalin('base', 'Settings');
yM = ImageInfo.ImInfo.yNumVox; %HO 7/8/2010
xM = ImageInfo.ImInfo.xNumVox; %HO 7/8/2010
xyum = ImageInfo.ImInfo.xyum; %HO 7/8/2010
...
%% Find territory
    ...
    %DrawRays %I don't know what this is for.
...
%%   Collect Data
    ...
    image(CA4),title('UpLeft:P/A(0-.128dots/um2), UpRight:D/A(0-.425um/um2), LowLeft:skeleton(cyan)+dots(red), LowRight:P/D(0-.64dots/um)');
    CA.Arbor(a).CA4 = CA4;
...
%     end
    ...
    %saveas(gcf,[TPN 'images\PA_0-pt128PunctaPerUm2&DA_0-pt425UmPerUm2&PD_0-pt64PunctaPerUm_PlusTitle.tif']) %save figure with title
    
end % Run Arbors

%% Manage Information

%HO put these information under CA.Arbor 10/15/2011
%     CA.mDtA=CA.Arbor(1).MeanPoverA; %mean P/A is calculated WITHIN the territory.
%     CA.mDdA=mean(CA.Arbor(1).DendDist(CA.Arbor(1).Territory>0)); %mean D/A is calculated WITHIN the territory.
%     CA.mDDA=CA.mDtA/CA.mDdA; %mean P/D is calculated WITHIN the territory.
%     
...
%     CA.Im.ShowCell=ShowCell;
    ...
    %%Write images
%     clear Cell
%     Name=[TPN 'images\PA_0-pt255PunctaPerUm2.tif'];
%     if isempty(find(Name=='?'));
...
if size(CA.Arbor,2)>2 %This is for bistratified GC.
   ...
   %if isdir([TPN 'images\Bi'])==0, mkdir([TPN 'images\Bi']); end %HO 7/8/2010
    Name=[TPN 'Images\PoverA_Bi.tif'];
    saveas(gcf,Name)
%     if isempty(find(Name=='?'));
